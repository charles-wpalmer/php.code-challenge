    ...
    <form wire:submit.prevent="createBooking">
        <div class="grid gap-5">
            <div>
                ...
                @error('title')<p class="text-sm text-red-600 mt-2"></p>{{ $message }}@enderror
            </div>
            <div>
                <label class="input input-bordered flex items-center gap-2">
                    <input wire:model="start_time" id="start_time" type="datetime-local" class="grow" placeholder="Start date"/>
                </label>
                @error('start_time')<p class="text-sm text-red-600 mt-2"></p>{{ $message }}@enderror
            </div>
            <div>
                <label class="input input-bordered flex items-center gap-2">
                    <input wire:model="end_time" id="end_time" type="datetime-local" class="grow" placeholder="End date"/>
                </label>
                @error('end_time')<p class="text-sm text-red-600 mt-2"></p>{{ $message }}@enderror
            </div>
        </div>
